...
namespace ddml {
  ...
  G4ThreeVector PolyhedraBarrelGeometryParallelWorld::localDirection(G4FastTrack const& aFastTrack) const {
    ...
    if( DEBUGPRINT ) {
      ...
      std::cout << "  PolyhedraBarrelGeometryParallelWorld::localDirection  - symmetry = " << _nSymmetry <<  " pos0 = " << position
		<< " - dir = " << direction << " - E = "
		<< " - localDir = " << localDir
		<< energy << std::endl ;
      std::cout << "  PolyhedraBarrelGeometryParallelWorld::localDirection  - phi = " << atan2( localDir.y() , localDir.x() ) / M_PI * 180.
  ...
  void PolyhedraBarrelGeometryParallelWorld::localToGlobal(G4FastTrack const& aFastTrack,
    ...
    if( DEBUGPRINT ) {
      ...
      std::cout << "  PolyhedraBarrelGeometryParallelWorld::localToGlobal  - symmetry = " << _nSymmetry <<  " pos0 = " << position
    ...
    if( DEBUGPRINT ) 
      std::cout << "  PolyhedraBarrelGeometryParallelWorld::localToGlobal -  position " << position << " - direction " << direction
		<< " phiSec: " << phiSec
		<< " posR " << posR << " dirR " << dirR 
		<< std::endl ;
    ...
    /* THIS WAS FOR A LAYERED CALORIMETER STRUCTURE
