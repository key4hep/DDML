...
namespace dd4hep {
  ...
  namespace sim   {
    ...
    Geant4SensitiveAction<Geant4FullFastCalorimeter>::process(const G4Step* step,G4TouchableHistory*) {
      ...
      if( ! (m_readout.idSpec().decoder()->get(cell, "submodule") == m_userData.fSensitiveEMSlice) ) return true; // String used should be specified in the steering file!
      ...
      dd4hep::printout(dd4hep::DEBUG, "Geant4FullFastCalorimeter::process", "FULL step material: %s", h.volume(step->GetPreStepPoint())->GetLogicalVolume()->GetMaterial()->GetName());
      ...
      if ( !hit ) {
        ...
        dd4hep::printout(dd4hep::DEBUG, "Geant4FullFastCalorimeter::process", "Actual FULL Hit creation Pos global: (%f, %f, %f)",
...

    {
      ...
      // local to world only accepts type Position
      dd4hep::Position pre_cell_loc_pos(pre_pos_cell.x(), pre_pos_cell.y(), pre_pos_cell.z());
      Position pre_glob_pos = Pre_DetEl.nominal().localToWorld(pre_cell_loc_pos);

      // Printouts for debugging here
      //std::cout << "Pre DetEl Global Position: x: " << pre_glob_pos.x() << " y: " << pre_glob_pos.y() <<  " z: " << pre_glob_pos.z() << std::endl;
      dd4hep::printout(dd4hep::DEBUG, "Geant4FullFastCalorimeter::processFastSim", "Pre CellID shift DetEl Global Position: (%f, %f, %f)",
                  pre_glob_pos.x(), pre_glob_pos.y(), pre_glob_pos.z());
      ...
      Hit* hit = coll->findByKey<Hit>(cell);
      if ( !hit ) {
	      Geant4TouchableHandler   handler(h.touchable());  /// This is no longer used to find localToGlobal
        DDSegmentation::Vector3D pos = m_segmentation.position(cell);
        ...
        dd4hep::printout(dd4hep::DEBUG, "Geant4FullFastCalorimeter::processFastSim", "Post CellID shift DetEl Global Position: (%f, %f, %f)",
      ...
      // debugging printout for hit position
      //std::cout << "Actual Hit creation Pos: x: " << hit->position.x() <<" y: " << hit->position.y() << " z: " << hit->position.z() << std::endl;
      dd4hep::printout(dd4hep::DEBUG, "Geant4FullFastCalorimeter::processFastSim", "Actual Hit creation Pos: (%f, %f, %f)",
                  hit->position.x(), hit->position.y(), hit->position.z());
